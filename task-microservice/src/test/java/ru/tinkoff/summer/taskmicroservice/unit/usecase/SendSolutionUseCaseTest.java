...
public class SendSolutionUseCaseTest {
  ...
  private String email = "email@mail.com";
    ...
    public void setUp() {
        ...
        when(crudAdapter.getTask(task.getId())).thenReturn(task);
    }
    ...
    public void incorrectCode_Error() {
        ...
        verify(crudAdapter, never()).save(any(long.class),any());
    ...
    public void sendSolution_StatusPending() {
        ...
        var solution = getSolution(code);
        when(crudAdapter.save(any(long.class), any())).then(returnsLastArg());

        sut.execute(solution);

        verify(crudAdapter).save(any(long.class),attemptCaptor.capture());
